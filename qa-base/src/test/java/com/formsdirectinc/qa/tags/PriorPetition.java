...
public class PriorPetition extends BaseTag {
	...
	public PriorPetition priorPetitionInformation(String elementProperty,
			String appname, int previousPetitionCount, String petitionType,
			String wasItfiledOnBehalf) {
		...
		for (int i = 0; i < previousPetitionCount; i++) {
			...
			System.out.println("Prior Petition----------->" + elementProperty
					+ "[" + i + "]");
			previousPetitionName = new Name_FirstMiddleLast(driver,
					"applicationData", elementProperty + "[" + i + "].name");
			previousPetitionName.setFirstNameField(data
					.getProperty(elementProperty + "[" + i + "].name.first"));
			previousPetitionName.setLastNameField(data
					.getProperty(elementProperty + "[" + i + "].name.last"));

			previousPetitionDOF = new DateTag(driver, elementProperty + "[" + i
					+ "].date");
			previousPetitionDOF.selectDate(data.getProperty(elementProperty
					+ "[" + i + "].date"));

			previousPetitionPlace = new Address(driver, "applicationData-"
					+ elementProperty + "[" + i + "].place");
			previousPetitionPlace.setCity(data.getProperty(elementProperty
					+ "[" + i + "].place.city"));
			previousPetitionPlace.setState(data.getProperty(elementProperty
					+ "[" + i + "].place.state"));

			resultsOfFilling = new MultiRadioOptions(driver, "applicationData-"
					+ elementProperty + "[" + i + "].result");
			if (petitionType.equalsIgnoreCase("Approved")) {
				...
				filedOnBehalf = new YesNo(driver, elementProperty + "[" + i
						+ "].wasPendingPetition");
			...
			if (i < previousPetitionCount - 1) {
				CommonInputClasses addAnotherPetition = new CommonInputClasses(
						driver);
	...
	public PriorPetition otherPetitionInformation(String elementProperty,
			String appname, int otherPetitionCount, String relationShipType) {
		...
		for (int i = 0; i < otherPetitionCount; i++) {
			...
			otherPetitionerName = new Name_FirstMiddleLast(driver,
					"applicationData", elementProperty + "[" + i + "].name");
			otherPetitionerName.setFirstNameField(data
					.getProperty(elementProperty + "[" + i + "].name.first"));
			otherPetitionerName.setLastNameField(data
					.getProperty(elementProperty + "[" + i + "].name.last"));

			relationshipFilling = new MultiRadioOptions(driver, elementProperty
					+ "[" + i + "].relationShip");
	...
	public PriorPetition otherPetitionInformationSecondPart(
			String elementProperty, String appname, int otherPetitionCount,
