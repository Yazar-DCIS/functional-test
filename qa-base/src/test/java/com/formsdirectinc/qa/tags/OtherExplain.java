...
public class OtherExplain extends BaseTag {
	...
	protected String otherTextFieldExpression = "%s-%s.other";
	...
	public OtherExplain setExplainationForDifferentAddress(String reason) {
		explain = driver.findElement(By.id(String.format(
				differentAddressExpression, beanName, property)));
		explain.clear();
		explain.sendKeys(reason);
		return this;
	}

	public OtherExplain setOtherText(String otherUSCIS_Office) {
		otherField = driver.findElement(By.id(String.format(
				otherTextFieldExpression, beanName, property)));
		otherField.clear();
		otherField.sendKeys(otherUSCIS_Office);
		return this;
	}
