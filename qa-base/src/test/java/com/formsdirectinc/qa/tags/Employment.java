...
public class Employment extends BaseTag {
	...
	public Employment employmentInformation(String elementProperty,
			String appname, String isThisCurrentOccupation,
			String occupationCountry) {

		PropertyResource propertyFile = new PropertyResource();
		Properties data = propertyFile.loadProperty(appname);

		employerInformation = new Employment(driver, "applicationData-"
				+ elementProperty);
		employerInformation.setEmployerName(data.getProperty(elementProperty
				+ ".employerName"));
		employerInformation.setEmployerOccupation(data
				.getProperty(elementProperty + ".occupation"));

		employerAddress = new Address(driver, "applicationData-"
				+ elementProperty + ".employerAddress");

		if (occupationCountry.equalsIgnoreCase("USA")) {
			employerAddress.setCountryField(data.getProperty(elementProperty
					+ ".employerAddress.countryUSA"));
			employerAddress.setApartmentTypeAndNumber(
					data.getProperty(elementProperty
							+ ".employerAddress.aptName"),
					data.getProperty(elementProperty
							+ ".employerAddress.aptNum"));
			employerAddress.setState(data.getProperty(elementProperty
					+ ".employerAddress.state"));
		} else if (!occupationCountry.equalsIgnoreCase("USA")) {
			employerAddress.setCountryField(data.getProperty(elementProperty
					+ ".employerAddress.country"));
		}
		employerAddress.setStreet(data.getProperty(elementProperty
				+ ".employerAddress.street"));
		employerAddress.setCity(data.getProperty(elementProperty
				+ ".employerAddress.city"));
		employerAddress.setZipcode(data.getProperty(elementProperty
		...
		if (isThisCurrentOccupation.equalsIgnoreCase("yes")) {
			...
			employmentDates = new DateTag(driver, elementProperty
					+ ".dateRange");
			employmentDates.setStartDay(data.getProperty(elementProperty
					+ ".dateRange.fromDate"));
			employmentDates.setStartMonth(data.getProperty(elementProperty
					+ ".dateRange.fromMonth"));
			employmentDates.setStartYear(data.getProperty(elementProperty
					+ ".dateRange.fromYear"));
			...
			employmentDates.setEndDay(data.getProperty(elementProperty
					+ ".dateRange.toDate"));
			employmentDates.setEndMonth(data.getProperty(elementProperty
					+ ".dateRange.toMonth"));
			employmentDates.setEndYear(data.getProperty(elementProperty
					+ ".dateRange.toYear"));
		}
	...
	public Employment employmentAuthorizationInformation(
			String elementProperty, String appname, int workAuthorizationCount) {
		...
		for (int i = 0; i < workAuthorizationCount; i++) {
			...
			employerInformation = new Employment(driver, "applicationData-"
					+ elementProperty + "[" + i + "]");
			employerInformation.setUSCISOfficeAppliedInfo(data
					.getProperty(elementProperty + "[" + i
							+ "].USCISOfficeApplied"));
			employerInformation.setResultsOfApplicationInfo(data
					.getProperty(elementProperty + "[" + i
							+ "].resultOfApplication"));

			employmentDates = new DateTag(driver, elementProperty + "[" + i
					+ "].dateApplied");
			employmentDates.selectDate(data.getProperty(elementProperty + "["
					+ i + "].dateApplied"));

			if (i < workAuthorizationCount - 1) {
				CommonInputClasses addAnotherWorkAuthorization = new CommonInputClasses(
						driver);
