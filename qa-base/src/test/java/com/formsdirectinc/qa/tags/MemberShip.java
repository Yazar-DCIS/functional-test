...
public class MemberShip extends BaseTag {
	...
	public MemberShip setMembershipInformation(String elementProperty,
			String appname, int membershipCount) {
		...
		for (int i = 0; i < membershipCount; i++) {
			...
			MemberShip memberInformation = new MemberShip(driver,
					"applicationData-" + elementProperty + "[" + i + "]");
			memberInformation.setNameOfOrganization(data
					.getProperty(elementProperty + "[" + i
							+ "].nameOfOrganization"));
			memberInformation.setTypeOfOrganization(data
					.getProperty(elementProperty + "[" + i
							+ "].typeOfOrganization"));

			Address membershipLocation = new Address(driver, "applicationData-"
					+ elementProperty + "[" + i + "].location");
			membershipLocation.setCity(data.getProperty(elementProperty + "["
					+ i + "].location.city"));
			membershipLocation.setCountryField(data.getProperty(elementProperty
					+ "[" + i + "].location.country"));

			DateTag membershipFromDate = new DateTag(driver, elementProperty
					+ "[" + i + "].dateRange.fromDate");
			membershipFromDate.selectDate(data.getProperty(elementProperty
					+ "[" + i + "].dateRange.fromDate"));

			DateTag membershipToDate = new DateTag(driver, elementProperty
					+ "[" + i + "].dateRange.toDate");
			membershipToDate.selectDate(data.getProperty(elementProperty + "["
					+ i + "].dateRange.toDate"));
