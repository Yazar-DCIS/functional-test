...
public class SelectElement extends BaseTag {
	...
	public SelectElement selectInvisibleElementUsingXpath() {
		...
		int element_size = driver.findElements(
				By.xpath(String.format(elementName, property))).size();
		elementField = driver.findElements(
				By.xpath(String.format(String.format(elementName, property))))
				.get(element_size - 1);
		elementField.click();
		return this;
	}
	...
	public String getElementText(String tag, String type) {

		elementField = driver.findElement(By.cssSelector(tag + "[" + type
				+ "='" + property + "']"));
		String elementText = elementField.getText().replace("Application ID:",
				"Application.ID=");
		return elementText;
	}
}
