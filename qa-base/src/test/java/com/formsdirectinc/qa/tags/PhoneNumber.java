...
public class PhoneNumber extends BaseTag {
	...
	public PhoneNumber setAddnPhoneNumber(String number) {
		addnPhoneNumberTextField = driver.findElement(By.id(String.format(
				otherNumberOldExpression, beanName + "-" + property)));
		addnPhoneNumberTextField.clear();
		addnPhoneNumberTextField.sendKeys(number);
		return this;
	}

	public void setContactInformation(String countryCode, String areaCode,
			String number, String otherNumber) {

		setPhoneCountryOptions(countryCode);
		if (countryCode.equals("1")) {
			setPhoneCode(areaCode);
			setPhoneNumber(number);
		} else {
			setAddnPhoneNumber(otherNumber);
		}
	}
}
